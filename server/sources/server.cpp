...
void TcpServer::messageExchange(ClientClass newClient)
{
...
int TcpServer::acceptClient()
{
    ...
    newClient.newClientAddrSize = sizeof(newClient.client_addr);
    newClient.ClientSocket = accept(m_serverSocket, (struct sockaddr *)&newClient.client_addr, &newClient.newClientAddrSize);
...
void TcpServer::sendMsg(ClientClass newClient)
{
    puts(newClient.msgClient);
    if((send(newClient.ClientSocket, newClient.msgClient, newClient.number, 0)) < 0)
    {
...
std::string TcpServer::receiveMsg(ClientClass newClient)
{
     memset(&newClient.msgClient, 0, sizeof(newClient.msgClient));
     
     newClient.number = recv(newClient.ClientSocket, newClient.msgClient, sizeof(newClient.msgClient), 0);
     std::cout << "Receive from client: " << newClient.msgClient << std::endl;
     std::string check = std::string(newClient.msgClient);
     return check;
}          
...
{
    TcpServer::ClientClass newClient;
    newClient.newClientAddrSize = sizeof(newClient);
...
void TcpServer::newThreadForClient()
{
